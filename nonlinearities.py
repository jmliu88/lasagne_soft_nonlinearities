...
class SoftSigmoid(object):
    """ Sigmoid with temprature
    :math:`\\varphi(x) = \\frac{1}{1 + e^{-x}}`
    where :math:`K` is the total number of neurons in the layer, T is hand set temprature. This
    ...
    -------
    __call__(x)
        Apply the soft sigmoid function to the activation `x`.
    """
    def __init__(self, T=1):
        self.T = T

    def __call__(self, x):
        return theano.tensor.nnet.sigmoid(x / self.T)
...
class SoftTarget(object):
    ...
    :math:`\\varphi(\\mathbf{x})_j =
    \\frac{e^{\mathbf{x}_j/T}}{\sum_{k=1}^K e^{\mathbf{x}_k/T}}`
    where :math:`K` is the total number of neurons in the layer, T is hand set temprature. This
    activation function gets applied row-wise.
    ...
    -------
    __call__(x)
        Apply the soft target function to the activation `x`.
    """
    def __init__(self, T=1):
        self.T = T

    def __call__(self, x):
        return theano.tensor.nnet.softmax(x / self.T)
